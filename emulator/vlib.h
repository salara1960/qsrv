...
#define wait_ack_sec_def 15
#define send_period_park 60
#define send_period_move 6
#define next_try 10
...
typedef struct
{
    ...
    uint16_t port;              //9090
    char imei[size_imei];       //"imei=",//351580051430040
    uint8_t mode;               //"mode="//0
    int snd_park;               //"period_park="//30
    int snd_move;               //"period_move="//10
    int wait_ack;               //"wait_ack="//15
    int wait_before_new_connect;//"wait_before_new_connect=",//3
...
extern void TheS(int sig);
extern void prints(const char *func, char *st, uint8_t with);
